def ten_to_base(x, new_base,extend=None):
    """
    Converts Base10 to BaseX(defined by user). Adding of new bases past Base36 is possible using the extend argument.
    Each new list item will be the new symbol for that base starting at Base37.
    """
    import sys
    base_chart = {
...
def convert_base(x, original_base, new_base=10):
    """
    Converts any Base to any Base but is limited to Base2 through Base36. Default new base is set to 10. Extending to a
    new base is not yet available with this function.
    """
